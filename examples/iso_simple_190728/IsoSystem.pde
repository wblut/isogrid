class IsoSystem { 
  ...
  IsoSystem(double L, int I, int J, int K, double centerX, double centerY, int[] colors, int seed, 
    ...
    grid = new WB_SimpleIsoHexGrid(centerX, centerY, L, L);
    int id=0;
    for (int i = 0; i < I; i++) {
      for (int j = 0; j < J; j++) {
        for (int k = 0; k < K; k++) {
    ...
    mapVoxelsToHexGrid(mini, maxi, minj, maxj, mink, maxk);
  }
...
*/
  ...
  public void mapVoxelsToHexGrid( int mini, int maxi, int minj, int maxj, int mink, int maxk) {
    grid.clear();
      ...
      for (int i = max(mini, 0); i < min(I, maxi); i++) {
        for (int j =max(minj, 0); j < min(J, maxj); j++) {
          for (int k = max(mink, 0); k < min(K, maxk); k++) {
            if(voxels[index(i,j,k)]) grid.addCube(i-I/2/*+(int)(I*(-0.25+0.5*home.noise(0.015*j)))*/, j-J/2, k-K/2/*+(int)(K*(-0.25+0.5*home.noise(0.015*j+25)))*/, colorIndices[index(i, j, k)]);
          }
        }
      }
  ...
  public void drawFaces(int zmin, int zmax) {
    ...
    if (FADE) {
      grid.drawTriangles(home, colors, zmin, zmax, 5*(I+J+K)/16+offset, -(I+J+K)/4+offset, -I/2, I/2, -J/2, J/2, -K/2, K/2);
    } else {
      grid.drawTriangles(home, colors, zmin, zmax);
    }
  }
}
